...
适用于 Windows 10/11 x64。安装与首次启动请保持联网。从 v0.2.0 起，安装版默认每天后台检查一次新版本，只提醒、不自动下载。看到提醒后点“一键更新”，下载完成后确认“安装并重启”即可；也可在“偏好设置 → 版本与更新”中关闭自动检查或手动更新。下载支持增量优先、完整包兜底及断点续传，更新保留用户数据。尚无一键更新入口的旧版先“停止并退出”，再手动运行新版安装包。`Source code (zip)` 是源码，不是安装包。
...
- **关键词批量采集**：抖音和小红书均支持；一次输入最多 20 个关键词，可设置每词作品数、采集深度、排序、发布时间、作品类型、互动门槛、每作品评论数、二级评论及媒体下载。已结束任务支持编辑配置、保留结果去重续跑和 Excel 导出。小红书使用主站登录态；“页面可见但详情媒体未读取到”通常表示笔记访问参数或页面结构不兼容，不应单凭该错误判定账号失效。
...
- **自定义监控频率**：作品、评论和弹幕监控的新建、编辑均可选择秒级预设，或选择“自定义”输入数值及“秒 / 分钟”。范围为 1–86400 秒；分钟可填小数（如 1.5 分钟 = 90 秒），换算后须为整秒。切换单位保持实际时长，已有分钟配置不变；弹幕仍可跟随全局间隔。任务耗时、错峰和账号限速仍会影响实际执行时间。
...
- 抖音本账号功能可在「设置 → 采集与运行 → 抖音本账号功能通道」中分别选择：手动刷新资料、粉丝列表、私信会话同步和本账号作品弹幕支持 `api`、`browser`、`hybrid`；发布当前使用浏览器。`api` 模式不会打开浏览器，`hybrid` 仅在直连未取得有效结果时回退，失败不会清空已有快照。
- 抖音可向已有会话发送私信，也可输入对方抖音号、主页链接或 `sec_uid` 发起新私信。可见抖音号会先通过抖音用户搜索精确匹配，再转换为 IM 内部 UID 建立会话，不能把纯数字抖音号直接当作内部 UID。
- 抖音 `api` 私信使用网页端 IM protobuf 协议建会和发送，并校验业务回执；建会、发送或回执状态不确定时，任务会保留为待确认且不会自动重试或切换浏览器重复发送。重新登录后应先同步并检查会话历史，再决定是否手动重试。
...
- 浏览器通道的私信同步、打开会话与发送复用每个账号唯一的后台工作标签页；Chrome 默认最小化启动，不会为每次操作新建或抢占前台窗口。点击“打开浏览器收发”时才主动恢复到前台。
